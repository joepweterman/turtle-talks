...
async function transcribe(wavPath, lang, baseDir) {
  ...
  const modelName = process.env.WHISPER_MODEL || 'ggml-medium.bin';
  const model = whisperDirs(baseDir)
    .map((d) => path.join(d, modelName))
    .find((p) => fs.existsSync(p));
  if (!exe || !model) {
    throw new Error(`whisper.cpp not set up (looked in ${path.join(baseDir, 'whisper')} and ~\\MeetingNotes\\whisper) — run "npm run setup" first.`);
  }
