...
// Merge timestamped, labeled segments into speaker turns.
// segments: [{label, track, fromMs, toMs, text}] with absolute offsets.
function buildTurns(segments) {
  const sorted = [...segments].sort((a, b) => a.fromMs - b.fromMs);
  const turns = [];
  for (const seg of sorted) {
    const label = seg.label || seg.track;
    const last = turns[turns.length - 1];
    if (last && last.label === label) {
      last.text += ` ${seg.text}`;
    } else {
      turns.push({ label, track: seg.track, fromMs: seg.fromMs, text: seg.text });
    }
  }
  return turns;
}
...
function turnsToMarkdown(turns) {
  const spoken = new Set(turns.map((t) => t.label));
  if (spoken.size <= 1) {
    // one voice — labels add nothing
    return turns.map((t) => t.text).join('\n');
  }
  return turns
    .map((t) => `**${t.label} · ${fmtMs(t.fromMs)}** ${t.text}`)
    .join('\n\n');
}
...
// Split a saved note back into its parts, for re-summarizing.
function parseNote(md) {
  const title = (md.match(/^# (.+)$/m) || [])[1] || '';
  const lang = md.slice(0, 400).includes('Nederlands') ? 'nl' : 'en';
  const transcript = (md.split(/## Transcript/i)[1] || '').trim();
  return { title: title.trim(), lang, transcript };
}

module.exports = { buildMarkdown, prettyStamp, buildTurns, turnsToMarkdown, speakerLabels, fmtMs, filterEcho, parseNote };
